...
import type { CategoryFilters, SortOption } from "@/lib/category-filters";

export function CategoryView({
  category, otherCategories, filters, facets, initialItems, nextCursor, total, query,
}: {
  category: CategoryDTO;
  otherCategories: CategoryDTO[];
  ...
  const activeSub = category.subcategories.find((subcategory) => subcategory.slug === filters.sub);
  ...
  const navigate = (next: CategoryFilters) => {
  ...
  const sidebar = (
    <aside className="space-y-6">
      <div>
        <h3 className="font-display font-bold mb-3">{category.emoji} {category.name}</h3>
        <ul className="space-y-1.5">
          <li>
            <button
              type="button"
              onClick={() => navigate({ ...filters, sub: undefined })}
              className={`w-full text-left px-3 py-2 rounded-xl text-sm font-medium transition ${!activeSub ? "bg-primary text-primary-foreground" : "hover:bg-blush"}`}
            >
              All {category.name}
            </button>
          </li>
          {category.subcategories.map((subcategory) => (
            <li key={subcategory.slug}>
              <button
                type="button"
                onClick={() => navigate({ ...filters, sub: subcategory.slug })}
                className={`w-full text-left px-3 py-2 rounded-xl text-sm font-medium transition ${activeSub?.slug === subcategory.slug ? "bg-primary text-primary-foreground" : "hover:bg-blush"}`}
              >
                {subcategory.name}
              </button>
            </li>
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-10">
      <nav className="flex items-center gap-1 text-xs text-muted-foreground mb-4" aria-label="Breadcrumb">
        ...
        {activeSub && (
          <>
            <ChevronRight className="h-3 w-3" />
            <span className="text-foreground font-semibold">{activeSub.name}</span>
          </>
        )}
      </nav>

      <div className="mb-8">
        <h1 className="font-display text-4xl sm:text-5xl font-black">{activeSub ? activeSub.name : category.name}</h1>
        <p className="text-muted-foreground mt-2">
          {activeSub
            ? `${category.name} • ${activeSub.name} — soft, cute, and made for daily joy.`
      ...
      <div className="mt-16">
        <h3 className="font-display text-2xl font-black mb-4">Also browse</h3>
        <div className="flex gap-3 flex-wrap">
          {otherCategories.map((item) => (
            <Link key={item.slug} href={`/category/${item.slug}`} className="btn-ghost-pink">{item.emoji} {item.name}</Link>
          ))}
        </div>
      </div>
    </div>
